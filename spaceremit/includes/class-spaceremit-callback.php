...
class SpaceRemit_Callback {
    ...
    public function __construct() {
        // Handle callback routing via query var
        add_action('init', array($this, 'handle_callback'));

        // Register query var ?spaceremit_callback=1
        add_filter('query_vars', array($this, 'add_query_vars'));

        // Catch plain GET returns and redirect to thank-you
        add_action('template_redirect', array($this, 'maybe_handle_get_return'));
    }
    ...
    public function handle_callback() {
        ...
        // Browser return via POST from payment page
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['SP_payment_code'])) {
            $this->handle_payment_callback();
            return;
        }

        // Server-to-server webhook (JSON)
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['SP_payment_code'])) {
            $this->handle_webhook_notification();
            return;
        }

        // For plain GET we do nothing here; maybe_handle_get_return() will redirect
        return;
    }

    /**
     * Redirect plain GET callback hits to thank-you page
     */
    public function maybe_handle_get_return() {
        if (!isset($_GET['spaceremit_callback'])) {
            return;
        }
